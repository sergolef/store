...
export class ShopService {
  ...
  getProducts(useCache = true): Observable<Pagination<Product[]>> {
    ...
    if(this.shopParams.seach) params = params.append("search", this.shopParams.seach);
  ...
  getProduct(id: number){
    const product = [...this.productCache.values()]
      .reduce((acumulator, paginatedRes) => {
        return {...acumulator, ...paginatedRes.data.find(x => x.id ===id)}
      }, {} as Product)
